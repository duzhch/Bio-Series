...
# ==========================================
# 2. Components: Rich Prior & Transformer
# ==========================================

class RichPriorGenerator(nn.Module):
    def __init__(self, delta_dim, gene_dim, out_dim=64):
        super().__init__()
        self.delta_compress = nn.Linear(delta_dim, 32)
        self.gene_compress = nn.Linear(gene_dim, 32)
        self.fuse = nn.Sequential(
            nn.Linear(64, out_dim),
            nn.LayerNorm(out_dim),
            nn.ReLU(),
            nn.Linear(out_dim, out_dim),
            nn.Tanh() # Tanh to allow negative weights in embeddings
        )
    def forward(self, d_emb, g_emb):
        d = F.relu(self.delta_compress(d_emb))
        g = F.relu(self.gene_compress(g_emb))
...
class GenomicTransformerV2(nn.Module):
    def __init__(self, block_size, in_channels=64, d_model=64, nhead=4, num_layers=2):
        super().__init__()
        
        # Local Conv: [Batch, in_channels, Block_Size] -> [Batch, d_model, 1]
        # In V13, in_channels is 64 (the prior dimension)
    ...
    def forward(self, x_folded):
        # x_folded: [B*N_Blk, d_model, Block_Size]
        return self.local_conv(x_folded).squeeze(-1)

# ==========================================
# 3. Bio-Master V12 (Rich Embeddings)
# ==========================================

class BioMasterV12(nn.Module):
    def __init__(self, delta_E, gene_E, num_snps, num_pcs, block_size=100):
        ...
        # Prior Generator now outputs d_model (64) instead of 1
        self.prior_gen = RichPriorGenerator(delta_E.shape[1], gene_E.shape[1], out_dim=self.d_model)
        
        self.genomic_transformer = GenomicTransformerV2(
    ...
    def forward(self, X_snps, X_pcs):
        ...
        # Path C: Deep (Transformer with Rich Priors)
        # priors: [N_SNPs, d_model]
        ...
        # Reshape Genotypes: [B, N_Blocks, Block_Size]
        g_blocks = X_p.view(B, self.n_blocks, self.block_size)
        
        # Reshape Priors: [N_Blocks, Block_Size, d_model]
        p_blocks = P_p.view(self.n_blocks, self.block_size, self.d_model)
        
        # Expand Priors to Batch: [B, N_Blocks, Block_Size, d_model]
        p_blocks_exp = p_blocks.unsqueeze(0).expand(B, -1, -1, -1)
        
        # Interaction: Element-wise multiplication (Scaling)
        # g_blocks is [B, N_Blk, Blk_Size], unsqueeze to [B, N_Blk, Blk_Size, 1]
        # Result: [B, N_Blocks, Block_Size, d_model]
        interaction = g_blocks.unsqueeze(-1) * p_blocks_exp
        
        # Fold for Conv1d: [B * N_Blocks, d_model, Block_Size]
        # Permute needed: (B, N_Blk, Blk_Size, d_model) -> (B*N_Blk, d_model, Blk_Size)
        x_folded = interaction.view(B * self.n_blocks, self.block_size, self.d_model).permute(0, 2, 1)
        ...
        # Return prior sum for regularization (L1 on 64 dims might be too strong, maybe L2?)
        # Let's keep L1 but scaled.
...
def train(plink_prefix, pheno_file, train_ids, test_ids, trait, delta_path, gene_path, out_dir, 
    ...
    print(f"🚀 Training Bio-Master V12 (Rich Transformer) on {dev}")
    ...
    model = BioMasterV12(delta, gene, num_snps=X_tr.shape[1], num_pcs=P_tr.shape[1], block_size=100).to(dev)
