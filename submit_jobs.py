...
def normalize_ablation(ablation: str) -> str:
    value = "full" if ablation is None else str(ablation).strip().lower()
...
def main():
    ...
    ap.add_argument('--model', default='bio_master_v11', help='Model name passed to launcher')
    ap.add_argument('--ablations', default='full', help='Comma-separated ablation modes')
    ...
    ablation_list = []
    for raw_ablation in (x for x in args.ablations.split(',') if x.strip()):
        ablation = normalize_ablation(raw_ablation)
        if ablation not in SUPPORTED_ABLATIONS:
            supported = ", ".join(sorted(SUPPORTED_ABLATIONS))
            ap.error(
                f"Unsupported ablation '{raw_ablation}' normalized to '{ablation}'. "
        ...
        for trait in tr_list:
            ...
            for r in range(1, nrep + 1):
                ...
                for ablation in ablation_list:
                    ...
                    ablation_tag = re.sub(r'[^A-Za-z0-9_.-]+', '_', ablation)
                    job = f'{ds}_{trait}_{model_tag}_{ablation_tag}_v5_{rep}'
                    ...
                    submit_lines.append(
                        f"# run-all --config {config_path} --dataset {ds} --trait {trait} --rep {rep} "
                        f"--model {args.model} --ablation {ablation}"
                    )
